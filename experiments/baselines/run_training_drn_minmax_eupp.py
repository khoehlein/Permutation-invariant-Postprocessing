...
from experiments.baselines.pp_drn_minmax import drn_pp
import pandas as pd
...
def main():
    ...
    parser.add_argument('--training:num-epochs', default=1000, type=int)
    parser.add_argument('--model:discriminator:num-updates', default=4, type=int)
    parser.add_argument('--model:discriminator:p-degree', default=12, type=int)
    parser.add_argument('--training:patience', default=10, type=int)
    parser.add_argument('--training:batch-size', default=1024, type=int)
    ...
    pred = drn_pp(
        ...
        nn_ls={
            ...
            'n_discriminator_updates': args['model:discriminator:num_updates'],
            'p_degree_discriminator': args['model:discriminator:p_degree'],
            'emb_dim': args['model:station_embedding'],
            'lay1': args['model:num_channels'],
            'nn_verbose': True,
            'n_epochs': args['training:num_epochs'],
            'n_batch': args['training:batch_size'],
